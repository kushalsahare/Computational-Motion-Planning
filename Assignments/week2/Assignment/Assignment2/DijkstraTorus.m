...
while true
    ...
    if(i>1 && i<= nrows)
        ...
        if(map(N(1))~=2 && map( N(1)) ~=3 && map(N(1))~=5 && distances(N(1))>min_dist+1)
          
                distances(N(1))=min_dist+1;
                parent(N(1))=current;
                map(N(1))=4;
...
if(i>=1 && i< nrows)
       ...
       N(2) = sub2ind(size(map), i+1, j);
        ...
        if(map(N(2))~=2 && map( N(2)) ~=3 && map(N(2))~=5 && distances(N(2))>min_dist+1)
          
                distances(N(2))=min_dist+1;
                parent(N(2))=current;
                map(N(2))=4;
...
if (j>1 && j<=ncols)
        ...
        if(map(N(3))~=2 && map( N(3)) ~=3 && map(N(3))~=5 && distances(N(3))>min_dist+1)
          
                distances(N(3))=min_dist+1;
                parent(N(3))=current;
                map(N(3))=4;
...
if (j>=1 && j<ncols)
        ...
        N(4) = sub2ind(size(map), i, j+1);
            ...
            if(map(N(4))~=2 && map( N(4)) ~=3 && map(N(4))~=5 && distances(N(4))>min_dist+1 )
                
            distances(N(4))=min_dist+1;
            parent(N(4))=current;
            map(N(4))=4;
